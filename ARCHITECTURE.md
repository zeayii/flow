...
- 业务执行采用顺序编排：顶层任务串行执行，目录内文件顺序处理
- 不使用协程并发调度任务/文件（不再使用任务队列 + worker 协程模型）
- 单文件复制内部仍使用异步流式 IO，以保持上传/下载语义与断点续传能力
- 呈现层采用双线程：`UI 线程` 负责渲染与输入，`日志线程` 负责日志聚合与落盘
- 业务到 UI/日志通过有界队列无阻塞投递，队列满时丢弃最旧项，避免反压业务路径
